...
public class UserServiceImpl implements UserService {
    ...
    public User getUserByNameAndPwd(User user){
        String sql="select * from user where username=? and password=? and visable=1";
        //1.执行查询
    ...
    public List<User> getAllUsers() {
        String sql = "select * from user WHERE visable=1 ";
    ...
    public User findUserById(String id) {
        User user = new User();
        String sql = "select * from user where id=? AND visable=1";
        try {
...
//        String sql = "DELETE FROM USER WHERE id=?;";
        String sql = "UPDATE USER SET visable = visable^1 WHERE id=?";
    ...
    public Integer getUserCount() {
        String sql = "select count(*) from user WHERE visable=1";
        return jdbcTemplate.queryForObject(sql, Integer.class);
    }

    public Integer getUserCount(Map<String, String[]> mp) {
        StringBuilder sql = new StringBuilder("select count(id) from user where 1=1 and visable=1 ");
...
//            sql.append("like '%");
    ...
    public List<User> findPageUserLimit(Integer currentPage, Integer pageSize) {
        ...
        String sql = "select * from user WHERE visable=1 limit ? , ?";
...
//            SELECT * FROM USER WHERE username LIKE '%三%' LIMIT 0,3
        StringBuilder sql = new StringBuilder("select * from user where visable=1 and 1=1 ");
        String []usernames = null;
        String []addresses = null;
        String []sexes = null;
        if(null != mp){
...
//            sql.append("like '%");
    ...
    @Override
    public int insertUserByMap(Map<String, String[]> mp) {
        User user = new User();
        int ret = 0;
        try {
            BeanUtils.populate(user, mp);
//            String birth = request.getParameter("birthday");
            String[] birthdays = mp.get("birthday");
            String birth = (String) birthdays[0] == null ? "2020-9-8" : birthdays[0];
            user.setBirthday(new SimpleDateFormat("yyyy-MM-dd").parse(birth));
            System.out.println(user);
            user.setGender(user.getSex());
            ret = insertUserByUser(user);
    ...
    @Override
    public Integer remake() {
        String sql = "UPDATE USER SET visable = 1 WHERE visable=0";
        int ret = jdbcTemplate.update(sql);
        System.out.println("-------begin-------userServiceImpl-------");
        System.out.println("remake sql" + sql);
    ...
    public Integer findUserByUsername(String username) {
        ...
        int ret = jdbcTemplate.queryForObject(sql, Integer.class, username);
