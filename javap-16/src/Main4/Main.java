...
class Th implements Runnable {
    ...
    public void run() {
        synchronized (a) {
            System.out.println("fst get a");
            synchronized (b) {
                System.out.println("fst get b");
            }
        }
        synchronized (b) {
            System.out.println("sec get a");
            synchronized (a) {
                System.out.println("sec get b");
            }
        }
    }
}
...
public class Main {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        Th th = new Th();
        Thread th1 = new Thread(th); th1.start();
        Thread th2 = new Thread(th); th2.start();
        Thread.sleep(99999);
        if(debug) {
