...
//            System.out.flush();
    ...
    public void send_file(DataInputStream sock_in, DataOutputStream sock_out) throws IOException {
        int name_len = sock_in.readInt();
        System.out.printf("name_len:%d\n", name_len);
        System.out.flush();
        byte buf[] = new byte[4096<<2];
        read_fill(sock_in, buf, 0, name_len);
        String file_name = new String(buf, 0, name_len);
        System.out.printf("file_name:%s\n", file_name);
        System.out.flush();
        StringBuilder sb = new StringBuilder(root_path);
        sb.append(file_name);
        send_file(sb.toString(), sock_out);
    }
